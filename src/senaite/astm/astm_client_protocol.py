...
class ASTMClientProtocol(asyncio.Protocol):
    ...
    def on_ack(self, data):
        ...
        if self.is_outbound_connection_active:
            response = self._handle_ack_for_outbound_communication()
        return response
    ...
    def send_outbound_message(self, message_to_LIS: List[str]):
        if self.in_transfer_state:
            raise NotAccepted(f"Already in transfer")
    ...
    def _reset_outbound_connection(self):
        ...
        self.last_sent_outbound_message_idx = -1
        self.message_to_LIS = []
        logger.debug(f"Reseted outbound connection")
